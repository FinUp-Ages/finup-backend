...
class TransactionRecurrenceControllerTest {
  ...
  void unknownIdReturns404() throws Exception {
    ...
    when(transactionRecurrenceService.findById(id))
        .thenThrow(new ResourceNotFoundException("TransactionRecurrence", id));

    mockMvc
        .perform(get("/api/v1/transaction-recurrences/{id}", id))
        .andExpect(status().isNotFound())
